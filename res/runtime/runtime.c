...
/*
   Allocates a class object on the heap and initializes all fields to
   zero.

   Param n: The total number of bytes required to store all field
   variables in the class object. Typically this is the machine word
   size times the number of field variables.
...
/*
   Allocates an integer array on the heap an initializes
   all fields to zero and also initializes the length of
   the array.

   Param numberOfElements: The desired number of elements in
   the array.

   Returns: A pointer to array element zero. Given the returned
   pointer <var>p</var> and the machine word size <var>W</var>,
   element <var>i</var> is stored in memory location
   <var>p</var>+<var>i</var>&times;<var>W</var> and the length of the
   array is stored in memory location <var>p</var>-<var>W</var>.
*/
